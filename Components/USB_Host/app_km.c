...
/* Modified by Ender Chan on 2025/09/13 */
...
ENUM_START:
    ...
    if( s == ERR_SUCCESS )
    {
        ...
        #if 1
        /* 如果不是通用HID配置描述符的写法（如：HM61Z，其将通信类设备和HID设备混合写到了一起），当遇到这种情况需进行特定的解析，将HID设备描述符分离出来 */
        if (RootHubDev[ usb_port ].bType == 0xff) {

            printf("The current device may be a HID device containing other types of devices, and specific parsing is required.\r\n");

            uint16_t index = 9;
            uint16_t itf_num;
            uint16_t cfg_len, cfg_itf_num;
            uint8_t  if_len, if_cls;
            uint8_t  Com_Buf_temp[ DEF_COM_BUF_LEN ];

            cfg_len = ( (PUSB_CFG_DESCR_LONG)Com_Buf )->cfg_descr.wTotalLength;
            cfg_itf_num = ( (PUSB_CFG_DESCR_LONG)Com_Buf )->cfg_descr.bNumInterfaces;
            printf( "cfg_len: %d\r\n", cfg_len );
            printf( "cfg_itf_num: %d\r\n", cfg_itf_num );
            for (uint8_t i = 0; i < 50; i++) {
                if_len = ( (PUSB_ITF_DESCR)&Com_Buf[index] )->bLength;
                if (if_len != 9) {
                    index += if_len;
                } else {
                    if_cls = ( (PUSB_ITF_DESCR)&Com_Buf[index] )->bInterfaceClass;
                    if (if_cls != USB_DEV_CLASS_HID) {
                        index += if_len;
                        ++itf_num;
                    } else {
                        index -= 9;
                        cfg_len -= index;
                        cfg_itf_num -= itf_num;
                        memset(&Com_Buf_temp[0], 0, sizeof(Com_Buf_temp));
                        memcpy(&Com_Buf_temp[0], &Com_Buf[0], 9);
                        memcpy(&Com_Buf_temp[9], &Com_Buf[9 + index], (cfg_len - 9));
                        Com_Buf_temp[2] = cfg_len;
                        Com_Buf_temp[4] = cfg_itf_num;
                        memcpy(&Com_Buf[0], &Com_Buf_temp[0], sizeof(Com_Buf));
                        for( i = 0; i < cfg_len; i++ )
                        {
            ...
            /* 特定处理：直接识别成键盘HID接口 */
            for (uint8_t i = 0; i < cfg_itf_num; i++) {
                dev_intf_type[i] = 0;
                printf("Specific processing, directly identify as a Keyboard HID interface\r\n");
            }
            USBH_AnalyseType( DevDesc_Buf, Com_Buf, &RootHubDev[ usb_port ].bType );
        }
        #endif
...
GETREP_START:
            ...
            if( s == ERR_SUCCESS )
            {
                ...
                if (Com_Buf[0] == 0x06) {
                    dev_intf_type[num] = 1;
                    printf("Custom HID interface\r\n");
                } else if (Com_Buf[0] == 0x05) {
                    if (Com_Buf[3] == 0x00) {
                        dev_intf_type[num] = 1;
                        printf("Custom HID interface\r\n");
                    } else {
                        dev_intf_type[num] = 0;
                        printf("Keyboard HID interface\r\n");
                    }
                }
#endif
    ...
    if( DevDesc_Buf[ 15 ] )
    {
        ...
        if( s == ERR_SUCCESS )
        {
            ...
            for (uint8_t i = 0; i < 10; i ++) {
                product_name[i] = Com_Buf[2 + (i * 2)];
            }
            DUG_PRINTF( "USB Product:%s\r\n", product_name );
#endif
        }
        else
        {
                        ...
                        if( HostCtl[ index ].Interface[ intf_num ].InEndpTimeCount[ in_num ] >= HostCtl[ index ].Interface[ intf_num ].InEndpInterval[ in_num ] )
                        {
                            ...
                            if( s == ERR_SUCCESS )
                            {
                                ...
                                if (!dev_intf_type[intf_num]) {
                                    ...
                                    if (len == 8) {             /* 不带report id的六键无冲 */
                                        key_cnt = 0;
                                    } else if (len == 8 + 1) {  /* 带report id的六键无冲 */
                                        key_cnt = 1;
                                    } else {                    /* 全键无冲暂时只支持不带report id的情况 */
                                        key_cnt = 0;
                                    }
                                    if (square_wave_flag) {
                                        for(uint8_t cnt = key_cnt; cnt < len; cnt++ ) {
                                            if (Com_Buf[cnt] != 0) {
                                                key_trigger_cnt++;
                                                square_single_press_flag = 0;
                                                key_cnt = cnt;
                                                key_code = Com_Buf[cnt];
                                                key_sta = 1;
                                                printf("press keycode: %2x\r\n", key_code);
                                                break;
                                            } else {
